...
export default function Partniership() {
  const data = [{
    ...
    partnershipsText: "When pairing the CORE corporate logo or any other CORE logo with another logo, a thin line is used to separate the two logos.",
    partners: "Partners",
    partnersItems: [
      {
          ...
          <div data-aos="fade-up" className="guidline__flex">
            {item.partnersItems.map((list, listIndex) => (
              <div className="guidline__half wdth340" key={listIndex}>
                <div className="guidline__half-ltl-ttl">
                  {list.title}
                </div>
                <div className="guidline__half-ltl-dscr">
                  {list.text}
                </div>
              </div>
            ))}
