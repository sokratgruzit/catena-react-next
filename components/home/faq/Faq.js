...
const Faq = ({ faqs }) => {
  ...
  if (window.innerWidth > 1240) {
    microSchemes = [[1, 2, 11, 12, 13, 14, 15, 16, 17, 23, 24]];
  }

  if (window.innerWidth < 1240) {
    microSchemes = [[1, 2, 5, 6, 7, 8, 9, 10, 11, 12, 13, 18, 19, 20, 21, 22, 23, 24]];
  }
    ...
    <div className={`pT-180`}>
      <div className={`${styles.container} container`}>
        <div className={styles.bodyContainer}>
          ...
          <p className={`${styles.Title} delay1 tYAnimation ${pageReady ? 'animate' : ''}`}>
            Answers to frequently asked questions about the CATENA Network.
          </p>
        </div>
