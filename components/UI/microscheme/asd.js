...
const Microscheme = props => {
    ...
    let animateLines = (array) => {
        ...
        let stringInDots = '';
        let stringOutDots = '';
        let fullRange = Array.from({ length: 12 }, (_, i) => i + 1);
        let outElementsArray = fullRange.filter(number => !array.includes(number));
        ...
        if(array.length !== 0) {
            for(let i = 0; i < array.length; i++) {
                stringIn = stringIn + '.line' + (i + 1).toString() + (i !== array.length - 1 ? ',' : '');
                stringInDots = stringInDots + '.circle' + (i + 1).toString() + (i !== array.length - 1 ? ',' : '');
            }
